...
import (
	"sync"
	"time"
	"gobible/logmanager/cli/std/interface"
...
func NewRedisConnection(fn ...func(*Options)) (Connection, error) {
	...
	if redisInstance != nil {
		if _interface.CompareAnyValues(redisInstance.option, option) {
...
func (r *Redis) Exist(key string) (exist bool, err error) {
	...
	if err = _interface.GenericTypeConvert(reply, &exist); err != nil {
		return
	}
	return
}
...
func (r *Redis) TTL(key string) (ttl time.Duration, err error) {
	...
	if err = _interface.GenericTypeConvert(reply, &ttl); err != nil {
		return
	}
	return
}
